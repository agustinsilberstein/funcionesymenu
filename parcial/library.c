...
void setId(book bookArray[], int arrayLenght, int valueId)
    {
     int i;
     for(i=0;i<arrayLenght;i++)
     {
         bookArray[i].id=valueId+i;
...
void showArray(book bookArray[],int arrayLenght)
{
    ...
    for(i=0;i < arrayLenght; i++)
    {
        if(bookArray[i].status != 0)
        {
            printf("\n| %6d  | %-40s | %5d | %5d | %d",bookArray[i].code,bookArray[i].title,bookArray[i].authorId,bookArray[i].stock, bookArray[i].id);
        }
    }
